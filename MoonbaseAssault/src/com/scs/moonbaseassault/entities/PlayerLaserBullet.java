...
public class PlayerLaserBullet extends AbstractPlayersBullet implements INotifiedOfCollision {
	...
	protected void createSimpleRigidBody(Vector3f dir) {
		...
		if (USE_CYLINDER) {
			...
			laserNode = BeamLaserModel.Factory(game.getAssetManager(), origin, origin.add(dir.mult(.2f)), ColorRGBA.Pink, !game.isServer(), "Textures/greensun.jpg", Globals.LASER_DIAM);
		} else {
			Mesh sphere = null;
